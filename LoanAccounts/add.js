...
let deposit_fees = [];
let deposits_products = [];

async function getDepositInterests() {
...
async function getCustomersAccount() {
  ...
  request.onload = function() {
    ...
    if (request.status >= 200 && request.status < 400) {
        ...
        const customer_accounts = data.data;
        
        let parent_gl_select_el = document.getElementById("field-customer-account");
       	appendCustomerSelectOption(customer_accounts, parent_gl_select_el);
    }
  }
...
async function getDepositProducts() {
    ...
    request.onload = function() {
        ...
        if (request.status >= 200 && request.status < 400) {
            ...
            deposits_products = data.data;
            console.log(deposits_products);
            
            let parent_gl_select_el = document.getElementById("field-product-name");
            appendSelectOption(deposits_products, parent_gl_select_el);
        }
    }
...
async function createNewDepositProduct(e) {
  ...
  let request = await cbrRequest(`/deposit-product`, 'POST', true)
